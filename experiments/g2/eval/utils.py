...
def get_input_encoding(
    questions: list[str],
    generation_model,  # Changed from transformers.LlamaForCausalLM to support any model
...
def get_output_texts(
    ...
    for generation_text in generation_texts:
        generation_text = generation_text.replace(
            "<s> [INST]", "<s>[INST]"
        )  # for chat models (llama-2-chat-hf, mistral-instruct, etc.)
...
def generate_completions(
    ...
    **generation_kwargs
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        if isinstance(batch_input_ids, dict):
            # Support multiple model types (llama, mistral, etc.)
            if 'llama' in batch_input_ids:
                batch_input_ids = batch_input_ids['llama']
            elif 'mistral' in batch_input_ids:
                batch_input_ids = batch_input_ids['mistral']
            else:
                # Get the first value if model-specific key not found
                batch_input_ids = next(iter(batch_input_ids.values()))
        ...
        # directly because some tokenizers (e.g., llama, mistral) won't add space token before the first token.
...
def dexperts_generate_completions(
    ...
    **generation_kwargs
    ...
    for i in range(0, len(base_prompts), batch_size):
        ...
        if isinstance(base_batch_input_ids, dict):
            # Support multiple model types (llama, mistral, etc.)
            if 'llama' in base_batch_input_ids:
                base_batch_input_ids = base_batch_input_ids['llama']
            elif 'mistral' in base_batch_input_ids:
                base_batch_input_ids = base_batch_input_ids['mistral']
            else:
                # Get the first value if model-specific key not found
                base_batch_input_ids = next(iter(base_batch_input_ids.values()))
