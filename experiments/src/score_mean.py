...
def rm_and_tokenizer():
    ...
    # Restrict to GPUs 0-2, exclude GPU 3
    #max_memory = {i: "40GiB" for i in range(3)}  # GPUs 0, 1, 2
    #max_memory[3] = "0GiB"  # Exclude GPU 3
...
async def score_partition_rm(
    ...
    # Get the device for input placement (first device when model is split)
    if hasattr(rm, "hf_device_map") and rm.hf_device_map:
        input_device = next(iter(rm.hf_device_map.values()))
    else:
        input_device = rm.device
...
async def process_instances(instances, output_file, patience, seed_ratio):
    ...
    async with aio_open(output_file, "w", buffering=1) as f:
        ...
        async def process_single_instance(instance):
            async with semaphore:
                ...
                # Calculate how many seed elements to skip
                num_seeds = int(len(instance["generations"]) * seed_ratio)
                # Skip seeds when calculating utility
                if num_seeds < len(generation_scores):
                    scores_for_utility = generation_scores
                    # Weights should start from position num_seeds (patience discount continues from where seeds end)
                    weights = patience ** np.arange(0, len(instance["generations"]))
                    utility = np.average(scores_for_utility, weights=weights)
                else:
                    # All elements are seeds, utility is 0
                    utility = 0.0
                return {
                    **instance,
...
async def main():
    ...
    parser.add_argument(
        "--seed-ratio",
        help="Ratio of first elements to abandon as seeds (e.g., 0.3 to skip first 30%%).",
        type=float,
        default=0.0,
    )
    ...
    output_file = os.path.join(eval_dir, "scores_mean.jsonl")
    await process_instances(instances, output_file, args.patience, args.seed_ratio)
