import json
import re
from agents import OpenAIAgent, GeminiAgent, Llama2Agent, MistralAgent
import datetime
import os
        ...
        for example in dataset['Examples']:
            ...
            for round in range(self.rounds):
                ...
                for agent in self.agents:
                    ...
                    if is_first_round: 
                        # If only 1 round, use the "last round" prompt that asks for finalization
                        if is_last_round:
                            # Use the last round prompt format for single round discussions
                            last_round_prompt = question + self.discussion_prompt + " This is the last round of the discussion, please finalize and present a list of as many creative answers as possible. Please list the final response in 1. ... 2. ... 3. ... and so on. \n\n"
                            formatted_initial_prompt = agent.construct_user_message(agent_role_prompt + last_round_prompt)
                        else:
                        ...
                        init_result = {"item": object, "uses": uses_list, "Agent": agent.agent_name}
                        init_results.append(init_result)
                        # Also save to final_results if it's the only round
                        if is_last_round:
                            final_result = {"item": object, "uses": uses_list, "Agent": agent.agent_name}
                            final_results.append(final_result)
